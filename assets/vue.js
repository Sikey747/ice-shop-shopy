...
Component that was made reactive: `,
    ...
    function cloneVNode(vnode, extraProps, mergeRef = false) {
        ...
        const cloned = {
            ...
            children:
                patchFlag === -1 && isArray(children) ? children.map(deepCloneVNode) : children,
            target: vnode.target,
    ...
    function useCssVars(getter) {
        ...
        const updateTeleports = (instance.ut = (vars = getter(instance.proxy)) => {
            Array.from(document.querySelectorAll(`[data-v-owner="${instance.uid}"]`)).forEach(
                (node) => setVarsOnNode(node, vars)
            );
        });
        const setVars = () => {
